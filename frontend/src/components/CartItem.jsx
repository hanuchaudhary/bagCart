          ...
          if (cartElems[e.id] > 0) {
            return (
              <div key={idx} className=" grid grid-cols-4 lg:gap-10 gap-4 mt-10">
                ...
                <h1 className="text-center flex rounded-full select-none md:mx-4 gap-4  items-baseline  font-semibold">
                  <span
                    onClick={() => delFromCart(e.id)}
                    className="bg-gray-300 lg:h-7 lg:w-7 h-4 w-4 flex items-center justify-center  select-none lg:text-center rounded-full font-semibold"
                  >
                    -
                  </span>
                  {cartElems[e.id]}
